...
export default function SoloGameContainer({ userId, onExit }: SoloGameContainerProps) {
  ...
  const calculateAndSaveScore = (isVictory: boolean, questionsAsked: number, correctCount: number) => {
    // שליפת השם העדכני ביותר מה-localStorage למניעת "שחקן"
    const actualName = localStorage.getItem('trivia_user_name') || roomData.players[0].name || 'שחקן';
    
    // חישוב ניקוד לפי אפיון v4.2.0
    let score = isVictory 
      ? Math.max(1000, 10000 - (questionsAsked * 150)) 
      : (correctCount * 10);
    ...
    const finalResult = Math.round(score);

    // 1. שמירה ל-Firebase (שיאים עולמיים)
    ...
    // 2. שמירה ל-localStorage (שיאים מקומיים)
    const localScores = JSON.parse(localStorage.getItem('trivia_solo_highscores') || '[]');
    localScores.push({ name: actualName, score: finalResult, date: Date.now(), difficulty: difficultyLabel });
    localStorage.setItem('trivia_solo_highscores', JSON.stringify(localScores));
  ...
  return (
    <div style={{ position: 'relative', height: '100dvh', overflow: 'hidden' }}>
      ...
      {step === 7 && <VictoryStep winnerName={localStorage.getItem('trivia_user_name') || roomData.players[0].name} score={finalScore} onRestart={onRestart} />}
